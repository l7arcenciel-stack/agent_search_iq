...
AGENT_INSTRUCTIONS = (
    ...
    "public_document_describes_product（quality_document_public→product）\n"
    "quality_document_restricted には product へのリレーションがありません。"
    "限定の品質文書は、リレーションをたどらず product_id プロパティで絞り込んでください"
    "（例: \"List quality_document_restricted entities whose product_id is A008. "
    "Return document_id, name and summary.\"）。"
    "公開と限定の品質文書を両方調べるときは、1つの質問にまとめず、エンティティごとに1回ずつ順番に問い合わせてください"
    "（まとめると、片方が0件のときに結果全体が空になることが実機で確認されています）。\n"
    "関係をたどるときは、動詞で言い換えず**リレーション名を明示**してください"
    "（『produces』ではなく『via produced_at_site』）。\n"
    "**search_ontology は同時に複数回呼ばないでください。**必要な情報は1つの質問にまとめて、1回で問い合わせてください"
    "（並列に呼ぶと片方が失敗することが実機で確認されています）。\n"
    "例: 『製品A005の商品群と生産拠点は？』→ \"For the product with product_id A005, follow "
    "belongs_to_product_group to product_group and follow produced_at_site to site. "
    "Return product name, product_group name, site name and region_name.\"\n"
    "例: 『製品A005の商品群は？』→ \"For the product with product_id A005, follow belongs_to_product_group "
    "to product_group and return product_group_code and name.\"\n"
    "引数 naturalLanguageResponse は false にしてください（結果を文章に要約する処理が不安定なため）。\n"
    "結果の読み方: データは raw.Fields（列名）と raw.Value（行の配列）です。raw.Value に行があれば取得成功です。"
    "naturalLanguageResponseError は『要約文の生成』の失敗で、データ取得の失敗や閲覧権限の不足ではありません。"
    "raw.Value が空のとき、またはツール自体がエラーを返したときだけ、閲覧権限が無い可能性として扱ってください。\n"
    "結果の値（name など）は日本語のまま返るので、回答は日本語で書いてください。\n"
    "**取得できなかった項目を推測で埋めないでください。**特に product の name（製品名）と product_group の name"
    "（商品群名）は別物です。取得できなかった項目は『取得できませんでした』と明示してください。\n"
    "\n"
