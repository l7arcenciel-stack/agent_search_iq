...
### 権限設計：案Aは現在の設定をそのまま引き継げる

**案Aが差し替えるのは翻訳の箱だけで、データへの行き方は現在と同じ**
（`executeQuery` に**本人のトークン**で GQL を投げる）。
**その経路で権限が効くことは §20-5 で実機確認済み**（未所属ユーザーは `security configuration enforcement` で拒否、
所属ユーザーだけが限定文書を取得）。したがって**権限まわりのやり直しはゼロ**。

裏を返すと、**案Aでは権限の制約も変わらない**（レイクハウス単位のまま、部署をまたぐ関係も張れないまま）。
**案Aが解くのはコストだけ、案Bが解くのは権限の制約だけ**で、別々の問題。
**2つは排他ではない**——「翻訳を自前にする」と決めたうえで、行き先を GQL にするか SQL にするかの選択。
翻訳を自前にしておけば、あとから行き先だけ差し替えられる。

| | 現在 ／ 案A | 案B（SQL エンドポイント + RLS） |
|---|---|---|
| 見せ分けの単位 | **レイクハウス単位**（全か無か） | **行・列単位** |
| データの置き方 | 範囲ごとにレイクハウスとワークスペースを分ける | 1つのレイクハウス・1つのワークスペースでよい |
| 部署をまたぐ関係 | **張れない**（属性 `product_id` で代用） | **JOIN で普通に成立** |
| 権限の書き方 | Fabric のアイテム共有（**ポータル作業**。API が無い） | RLS の述語（**SQL で書ける**） |
| `ReadAll` | **必須** | **付けてはいけない**（SQL の RLS が迂回される） |
| 反映までの時間 | **最大2時間** | 即時 |
| 誰の権限で動くか | 本人（OBO トークン） | 本人 |

### 自前実装で守る原則：アプリ側で権限を判定しない

**「自前で作るなら権限チェックもアプリ側に書けばいい」は誤り。**
アプリ側のフィルタは**権限ではなく、ただの絞り込み**。コードにバグがあれば漏れるし、
別の経路（ポータル・SQL エンドポイント・OneLake）から直接アクセスされれば素通りする。

- **本人のトークンで問い合わせ、データ側（第3層）に判定させる構造は、案Aでも絶対に崩さない**
- 自前で書いてよいのは「**どのエンティティを問い合わせるか**」まで
- **「このユーザーに見せてよいか」を判断するコードは書かない**
- `ontology_schema.py` の Allowlist は**安全のための絞り込み**であって**権限ではない**。
  Allowlist に載っていても、権限が無ければ `security configuration enforcement` で拒否される——これが正しい状態
- 受け入れ条件：**太郎・次郎の差が、アプリのコードを1行も通らずに出る**こと。
  切り分けには §20-5 と同じやり方（`AZURE_CONFIG_DIR` を分けて本人で `az login` → 直接 `executeQuery`）を使う
...
4. `main.py` のツールを差し替え、デモ UI の4問で太郎・次郎の結果が現状と一致することを確認。
   **受け入れ条件は「2人の差が、アプリのコードを1行も通らずに出る」こと**（上記の原則）
