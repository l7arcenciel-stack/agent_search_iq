...
| ワークスペース名（限定） | `ws-agent-search-iq-restricted` | | 変える | **閲覧者は品質チームグループのみ**。限定データを公開側と同じワークスペースに置くと、閲覧者に SQL 経由で漏れる（operations_log §20） |
| レイクハウス名（限定） | `lh_restricted` | | 同じ | 限定用ワークスペースに置き、品質チームグループに **ReadAll 付きで共有（必須）**。グループへの共有で足りる（反映に最大2時間） |
...
- [x] ただし**デモユーザーをワークスペースの閲覧者にすると崩れた**（同じワークスペースの `lh_restricted` を SQL エンドポイントで読めた）。
  **`lh_restricted` を別ワークスペースに移したら回復**（2026-09-22、未所属ユーザーはインスタンスが 401）
