...
【既知の不具合】
  アイテム個別のサブリソース（/lakehouses/<id>/tables、/ontologies/<id>/getDefinition）が
  **404 を返すことがある**。同じコードで直前まで取得できていたのに突然返らなくなる。
  Graph 向けに az login --scope を実行した後に発生したため、トークンが関係している疑いがある。
  対処: `az login --tenant <TENANT_ID>` で入り直す。それでも直らなければポータルで目視確認する。
  このスクリプトはエラーを「0件」と誤読しないよう [??] で区別して表示する。
...
def main() -> None:
    ...
    if not target:
        ...
        todo.append(f"ワークスペース {WORKSPACE_NAME} を作成し、F2 容量に割り当てる")
...
def _report_ontology(wsid: str, oid: str, tok: str, lh_ids: dict, todo: list) -> None:
    ...
    if any("?" in str(w) for w in [1]) or len(ents) < 5:
        todo.append("03_create_ontology_definition.py を実行して定義を更新する")
